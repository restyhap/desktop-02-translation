...
impl KeyManager {
    ...
    pub fn save_key(
        ...
        let display_name = if display_name.is_empty() { service } else { display_name };
        ...
        stmt.bind((3, app_id.unwrap_or(""))).map_err(|e| e.to_string())?;
        stmt.bind((4, key)).map_err(|e| e.to_string())?;
        stmt.bind((5, sort)).map_err(|e| e.to_string())?;
        stmt.next().map_err(|e| format!("保存 API Key 失败: {}", e))?;
        Ok(())
    }
    ...
    pub fn get_key(app: &tauri::AppHandle, service: &str) -> Result<Option<String>, String> {
        let conn = sqlite::open(Database::get_db_path(app)).map_err(|e| e.to_string())?;
        Self::ensure_schema(&conn)?;
        let mut stmt = conn.prepare("SELECT api_key FROM api_keys WHERE service_name = ?")
            .map_err(|e| e.to_string())?;
        stmt.bind((1, service)).map_err(|e| e.to_string())?;
        match stmt.next() {
            Ok(sqlite::State::Row) => stmt.read::<String, _>(0).map(Some).map_err(|e| e.to_string()),
            Ok(_) => Ok(None),
            Err(e) => Err(e.to_string()),
        }
    }

    pub fn get_key_with_appid(app: &tauri::AppHandle, service: &str) -> Result<Option<(String, String)>, String> {
        let conn = sqlite::open(Database::get_db_path(app)).map_err(|e| e.to_string())?;
        Self::ensure_schema(&conn)?;
        let mut stmt = conn.prepare("SELECT api_key, app_id FROM api_keys WHERE service_name = ?")
            .map_err(|e| e.to_string())?;
        stmt.bind((1, service)).map_err(|e| e.to_string())?;
        match stmt.next() {
            Ok(sqlite::State::Row) => {
            ...
            match stmt.next() {
                Ok(sqlite::State::Row) => {
                    ...
                    records.push(ApiKeyRecord {
                        ...
                        display_name: if display_name.is_empty() { service_name } else { display_name },
                        app_id,
                        api_key: masked_key,
                        sort,
                    });
    ...
    pub fn delete_key(app: &tauri::AppHandle, service: &str) -> Result<(), String> {
        let conn = sqlite::open(Database::get_db_path(app)).map_err(|e| e.to_string())?;
        Self::ensure_schema(&conn)?;
        let mut stmt = conn.prepare("DELETE FROM api_keys WHERE service_name = ?")
            .map_err(|e| e.to_string())?;
        stmt.bind((1, service)).map_err(|e| e.to_string())?;
